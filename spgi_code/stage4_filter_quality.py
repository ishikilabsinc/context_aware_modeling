#!/usr/bin/env python3
"""
Stage 4: Filter and Balance High-Quality Samples (SPGI)

Filter stage3 output: remove fillers, balance SPEAK/SILENT, deduplicate.

When STAGE4_CHUNK_SIZE is set (e.g. 100_000), reads input in chunks, processes
each chunk (filter + dedup), and appends to the output JSONL so the full file
is never loaded into memory. Balancing is applied per chunk to approximate
global speak/silent ratio.

Input: json_dumps/stage3_categorized_samples.jsonl
...
from config import (
    ...
    STAGE4_MAX_SAMPLES,
    STAGE4_MAX_SAMPLES_PER_MEETING,
    ...
    STAGE4_MAX_CURRENT_TURN_CHARS,
    STAGE4_MAX_CONTEXT_TURNS,
    STAGE4_MAX_LINE_BYTES,
    STAGE4_CHUNK_SIZE,
    STAGE4_REQUIRE_ENGAGED_SILENT,
    STAGE4_SILENT_RECENCY_WINDOW,
    STAGE4_SILENT_MIN_SPEAKER_TURNS,
)

INPUT_FILE = JSON_DUMPS_DIR / 'stage3_categorized_samples.jsonl'
...
def is_too_long(sample: Dict) -> bool:
    """Exclude samples that exceed max length limits (saves memory, avoids OOM)."""
    if STAGE4_MAX_CURRENT_TURN_CHARS is not None:
        text = sample.get('current_turn', {}).get('text', '') or ''
        if len(text) > STAGE4_MAX_CURRENT_TURN_CHARS:
            return True
    if STAGE4_MAX_CONTEXT_TURNS is not None:
        ctx = sample.get('context_turns', [])
        if len(ctx) > STAGE4_MAX_CONTEXT_TURNS:
            return True
    return False


def is_trivial_silent(sample: Dict) -> bool:
    """
    Return True if this SILENT sample is low-quality — the target had no
    plausible reason to speak (not addressed, not recently active, barely
    present in the conversation).  Keeping only "engaged" SILENT samples
    makes the task harder and the training signal more informative.
    """
    if not STAGE4_REQUIRE_ENGAGED_SILENT:
        return False
    if sample.get('decision') != 'SILENT':
        return False

    # Keep if target was directly addressed (they had a strong cue but stayed silent)
    if sample.get('target_is_addressed'):
        return False

    target = sample.get('target_speaker')
    context_turns = sample.get('context_turns', [])

    # Keep if target spoke recently (within the recency window)
    recent = context_turns[-STAGE4_SILENT_RECENCY_WINDOW:]
    if any(t['speaker'] == target for t in recent):
        return False

    # Keep if target has spoken enough times in the full context
    total_target_turns = sum(1 for t in context_turns if t['speaker'] == target)
    if total_target_turns >= STAGE4_SILENT_MIN_SPEAKER_TURNS:
        return False

    # Target is disengaged → trivial silence, drop it
    return True
...
def _meeting_key(sample: Dict) -> str:
    """Key for meeting-scoped dedup (same meeting only); fallback if missing."""
...
def deduplicate_samples(samples: List[Dict]) -> List[Dict]:
    ...
    # Exact match only (backward compatible)
    if thresh >= 1.0:
        seen: Set[str] = set()
        deduped = []
        for s in sorted_samples:
            ...
            deduped.append(s)
        return deduped
    # N-gram similarity: only compare within same meeting; use last 5 context + current
    deduped = []
    # key: (meeting_id, decision) when same_meeting_only else decision
...
def apply_filters(samples: List[Dict]) -> List[Dict]:
    """Apply quality + context/short-query/gemini/silent-engagement filters (no dedup)."""
    out = []
    for s in samples:
        ...
        if STAGE4_GEMINI_CONFIDENCE_FILTER is not None and s.get('source') == 'gemini_inferred':
            if s.get('inference_confidence', 0) < STAGE4_GEMINI_CONFIDENCE_FILTER:
                continue
        if is_trivial_silent(s):
            continue
        out.append(s)
    return out


def cap_samples_per_meeting(samples: List[Dict], max_per_meeting: int) -> List[Dict]:
    """Keep at most max_per_meeting samples per meeting_id, preferring higher confidence."""
    if not samples or max_per_meeting is None or max_per_meeting <= 0:
        return samples
    by_meeting: Dict[str, List[Dict]] = defaultdict(list)
    for s in samples:
        mid = str(s.get('meeting_id', ''))
        by_meeting[mid].append(s)
    out = []
    for mid, lst in by_meeting.items():
        sorted_lst = sorted(lst, key=lambda x: confidence_to_score(x['confidence']), reverse=True)
        out.extend(sorted_lst[:max_per_meeting])
    return out


def process_chunk(chunk: List[Dict]) -> List[Dict]:
    """Filter (quality + advanced + max length), deduplicate, balance, and cap per meeting."""
    filtered = [s for s in chunk if not is_low_quality(s) and not is_too_long(s)]
    advanced = apply_filters(filtered)
    if STAGE4_ENABLE_DEDUPLICATION:
        advanced = deduplicate_samples(advanced)
    balanced = balance_speak_silent(advanced)
    if STAGE4_MAX_SAMPLES_PER_MEETING is not None:
        balanced = cap_samples_per_meeting(balanced, STAGE4_MAX_SAMPLES_PER_MEETING)
    return balanced
...
def main():
    ...
    chunk_size = STAGE4_CHUNK_SIZE

    if chunk_size is None or chunk_size <= 0:
        # Original behavior: load all, then filter, dedup, balance, write
        all_samples = []
        with open(INPUT_FILE, 'r') as f:
            for line in tqdm(f, desc="Loading samples", unit=" lines"):
                line = line.strip()
                if not line:
                    continue
                if STAGE4_MAX_LINE_BYTES is not None and len(line.encode('utf-8')) > STAGE4_MAX_LINE_BYTES:
                    continue  # skip huge lines without parsing
                all_samples.append(json.loads(line))
        print(f"Loaded {len(all_samples):,} samples")
        filtered = [s for s in all_samples if not is_low_quality(s) and not is_too_long(s)]
        print(f"After quality + max-length filter: {len(filtered):,}")
        advanced = apply_filters(filtered)
        if STAGE4_ENABLE_DEDUPLICATION:
            advanced = deduplicate_samples(advanced)
        ...
        if STAGE4_MAX_SAMPLES_PER_MEETING is not None:
            balanced = cap_samples_per_meeting(balanced, STAGE4_MAX_SAMPLES_PER_MEETING)
            print(f"After cap per meeting ({STAGE4_MAX_SAMPLES_PER_MEETING} max): {len(balanced):,}")
        else:
        ...
        print(f"✓ Saved {len(balanced):,} samples to {output_path} (json_run skipped for SPGI)")
        ...
        summary = {
            ...
            'filter_from_jsonl': {
                ...
                'source': "Filter by origin (e.g. 'spgi_heuristic'). SPGI uses heuristic only; inference_confidence is fixed.",
                'inference_confidence': "Filter with inference_confidence >= N (0-10). E.g. keep only rows with confidence >= 6.",
            },
        }
        with open(OUTPUT_DIR / 'filtering_summary.json', 'w') as f:
            json.dump(summary, f, indent=2)
    else:
        # Chunked: read chunk → filter, dedup, balance → write chunk → repeat (bounded memory)
        total_written = 0
        total_speak = 0
        total_silent = 0
        category_counts: Counter = Counter()
        chunk_num = 0
        max_per_meeting = STAGE4_MAX_SAMPLES_PER_MEETING
        meeting_write_count: Dict[str, int] = defaultdict(int)

        with open(output_path, 'w') as out_f:
            chunk: List[Dict] = []
            with open(INPUT_FILE, 'r') as f:
                for line in tqdm(f, desc="Reading input", unit=" lines"):
                    line = line.strip()
                    if not line:
                        continue
                    if STAGE4_MAX_LINE_BYTES is not None and len(line.encode('utf-8')) > STAGE4_MAX_LINE_BYTES:
                        continue  # skip huge lines without parsing (avoids OOM)
                    chunk.append(json.loads(line))
                    if len(chunk) >= chunk_size:
                        chunk_num += 1
                        balanced = process_chunk(chunk)
                        written_this_chunk = 0
                        for s in balanced:
                            mid = str(s.get('meeting_id', ''))
                            if max_per_meeting is not None and meeting_write_count[mid] >= max_per_meeting:
                                continue
                            out_f.write(json.dumps(add_filter_fields(s)) + '\n')
                            meeting_write_count[mid] += 1
                            total_written += 1
                        ...
                        chunk = []
                        tqdm.write(f"  Chunk {chunk_num}: wrote {written_this_chunk:,} samples (total so far: {total_written:,})")
            ...
            if chunk:
                ...
                for s in balanced:
                    ...
                    if s['decision'] == 'SPEAK':
                        ...
                        total_silent += 1
                    category_counts[s['category']] += 1
                tqdm.write(f"  Chunk {chunk_num}: wrote {len(balanced):,} samples (total: {total_written:,})")

        print(f"✓ Saved {total_written:,} samples to {output_path} (chunked, {chunk_num} chunks)")
        summary = {
            'total_samples': total_written,
            'speak_samples': total_speak,
            'silent_samples': total_silent,
        ...
        with open(OUTPUT_DIR / 'filtering_summary.json', 'w') as f:
            json.dump(summary, f, indent=2)
        # json_run skipped in chunked mode to avoid writing millions of small files
